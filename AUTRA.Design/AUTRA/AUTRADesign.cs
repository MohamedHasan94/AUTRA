...
namespace AUTRA.Design
{
    public class AUTRADesign
    {
        ...
        public AUTRADesign(Project project, ICodeDesigner code, List<Section> sections, List<LoadPattern> patterns, Bolt bolt)
        {
            ...
            if (project.loadCombination != null && project.loadCombination.Combo.Count != 0)
                Combos.Add(project.loadCombination);
            else
                CreateCombo("1*D + 1*L", new FactoredPattern() { Pattern = LoadPattern.DEAD, ScaleFactor = 1.0 }, new FactoredPattern() { Pattern = LoadPattern.LIVE, ScaleFactor = 1.0 });
